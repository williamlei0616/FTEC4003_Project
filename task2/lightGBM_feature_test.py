...
def train_and_evaluate(feature_to_add=None):
    """Train model with optional feature added back and return F1 score"""
    
    try:
        # Build list of features to drop (all dropped except the one we're testing)
        if feature_to_add:
            current_drops = [f for f in dropped_features if f != feature_to_add]
        else:
        ...
        print(f"Error processing {feature_to_add}: {str(e)}")
        return None


# Store results
results = {}
output_file = "feature_test_results.txt"

# Initialize output file
with open(output_file, "w") as f:
    f.write("Feature Add-Back Test Results\n")
    f.write("=" * 60 + "\n")
    f.write(f"Testing {len(dropped_features)} dropped features\n")
    f.write("Results will be updated as each feature is tested...\n\n")

# First, get baseline (no features added back)
print("=" * 60)
print("Testing BASELINE (no dropped features added back)")
print("=" * 60)
start_time = time.time()
baseline_f1 = train_and_evaluate(feature_to_add=None)
...
with open(output_file, "a") as f:
    ...
    f.write("=" * 60 + "\n\n")
    f.write("Individual Feature Add-Back Results:\n")
    f.write("-" * 60 + "\n")

print()

# Test each dropped feature
start_total = time.time()
for i, feature in enumerate(dropped_features[::-1]):  # Reverse order for variety
    print("=" * 60)
    print(f"[{i+1}/{len(dropped_features)}] Testing with feature: {feature}")
    print("=" * 60)
    ...
    if feature not in merged_df.columns:
        ...
        # Write to file
        with open(output_file, "a") as f:
            f.write(f"ADD {feature}: Not found in dataset\n")
        continue
    
    feature_start = time.time()
    f1_score = train_and_evaluate(feature_to_add=feature)
    ...
    if f1_score is not None:
        ...
        print(f"Time taken: {feature_time:.2f}s")
        
        # Write to file immediately
        with open(output_file, "a") as f:
            f.write(f"ADD {feature}: {f1_score:.4f} (diff: {diff:+.4f}) | Time: {feature_time:.2f}s\n")
    else:
        print(f"Failed to get F1 score for {feature}")
        with open(output_file, "a") as f:
            f.write(f"ADD {feature}: Failed\n")
    
    # Calculate ETA
    elapsed = time.time() - start_total
    avg_time = elapsed / (i + 1)
    remaining = (len(dropped_features) - i - 1) * avg_time
...
print(f"Average time per feature: {total_time/len(dropped_features):.1f}s")
...
with open(output_file, "a") as f:
    ...
    f.write(f"Average time per feature: {total_time/len(dropped_features):.1f}s\n")
    f.write(f"Baseline F1 Score: {baseline_f1}\n\n")
    f.write("Top 20 Features by F1 Score when ADDED BACK:\n")
    f.write("-" * 60 + "\n")
    
    for i, (feature, f1) in enumerate(sorted_results[:20]):
        diff = f1 - baseline_f1 if baseline_f1 else 0
        line = f"{i+1}. ADD {feature}: {f1:.4f} (diff: {diff:+.4f})"
        print(line)
        f.write(line + "\n")

# Print features that improved the score
print("\n" + "=" * 60)
print("FEATURES THAT IMPROVED F1 SCORE WHEN ADDED BACK:")
print("=" * 60)
improved_features = []
...
if improved_features:
    ...
    with open(output_file, "a") as f:
        f.write("\n" + "=" * 60 + "\n")
        f.write("FEATURES THAT IMPROVED F1 SCORE WHEN ADDED BACK:\n")
        f.write("=" * 60 + "\n")
        
        for feature, improvement in improved_features:
            line = f"ADD {feature}: +{improvement:.4f}"
            print(line)
            f.write(line + "\n")
    
    print(f"\nConsider adding back these {len(improved_features)} features!")
else:
    print("No features improved the score when added back.")
    with open(output_file, "a") as f:
        f.write("\nNo features improved the score when added back.\n")
